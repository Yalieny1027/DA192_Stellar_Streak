...
require_once '../../controller/msgSender.php'; //send Text message is being handled by this particular file
...
// TODO: Register script name , mobile  , email , password
 class Register{
   ...
   function generateOTP(){
	...
	//it will return random five digit OTP 
	//but we don't have any working SMS Sending Integration so we will replaceit with static OTP
	return 12345 ; 
  ...
  if (!validName($name)) {
    ...
    if($register->insert($name , $email , $mobile , $hashPassword)){
      ...
      if($register->insertToken($userId , $otp)){
        //send $otp
      //  sendTextMsg($mobile , $otp); //this funciton is responsible for sending OTP to Mobile number
        $response['error'] = false;
        $response['msg'] = "OTP has been sent to your mobile";
        echo json_encode($response);
        return;
